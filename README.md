...
To run the service itself, copy `.env.example` to `.env`, create the application's database role as described in [docs/database-roles.md](docs/database-roles.md), point `MIGRATION_DATABASE_URL` at the schema owner and `DATABASE_URL` at that role, then `npm run dev`. Migrations are applied on startup, as the owner. Without `MIGRATION_DATABASE_URL`, as in production, the service migrates nothing and refuses to start unless the database is already fully migrated. The service refuses to start if `DATABASE_URL` could alter an Audit record.
