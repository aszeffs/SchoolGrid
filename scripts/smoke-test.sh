...
# serve alongside the first: one checked above, and the one offering trials.
...
TRIALS_ORIGIN="http://localhost:${OWNERLESS_HOST_PORT}"
...
# the application's role, as production does. Given `trials`, it holds only that
# role and runs with TRIALS_ENABLED on, as the public showcase does.
#
...
if [ "$#" -gt 0 ]; then
  ...
  if ! SCHOOLGRID_ORIGIN="$ORIGIN" \
    SCHOOLGRID_TRIALS_ORIGIN="$TRIALS_ORIGIN" \
