...
describe("GET /api/health", () => {
  ...
  it("returns 404 for an unknown route without echoing the path back", async () => {
    const response = await server().client.get("/api/does-not-exist");
  ...
  it("refuses every unknown route identically", async () => {
    ...
    const first = await server().client.get("/api/schools/1/students");
    const second = await server().client.get("/api/totally-invented");
