...
describe("Courses and Class Offerings", () => {
  ...
  async function arrange(): Promise<World> {
    ...
    const alice = (await server().signIn(ALICE)).inSchool(northside.school.id);
    const bob = (await server().signIn(BOB)).inSchool(westbrook.school.id);
  ...
  describe("outside the caller's reach is the standard refusal", () => {
    ...
    // One world, every caller and every action tried within it: arranging a
    // world signs in several accounts, and each is hashed as slowly as it
    // should be.
