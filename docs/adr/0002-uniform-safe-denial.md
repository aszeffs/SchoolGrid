...
- **Sign-out may expire the cookie it was given.** A sign-out from the public origin carrying a session cookie gets the one refusal when no live Session stood behind it, but still expires that cookie, so a browser is not left holding one that grants nothing. The expiring cookie follows from the cookie the caller sent, not from whether a Session existed, so a caller learns only what it already knew it presented. A cross-origin sign-out gets the refusal without it (ADR-0004).
- **A refusal outside `/api` is revalidated rather than never stored.** Every response under `/api` carries `cache-control: no-store`; a refusal of any other path carries `no-cache`, as the web app's page does. The value follows from the decoded request path alone, never from whether a file or route exists there, so it reveals only which side of `/api` the caller asked for. A file the web app serves carries its own policy, which a refusal never does (#88).
